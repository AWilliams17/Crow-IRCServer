...
def create_endpoints(server, server_settings, ssl_settings):
    ...
    if server_config.SSLSettings.SSLEnabled:
        ...
        if ssl_key is None or ssl_cert is None:
            ...
            if ssl_only:
                print(
                    "Error: SSLOnly is enabled and no SSL endpoint was created, so there is no endpoint. Terminating.")
                exit()
        else:
            ssl_endpoint = serverFromString(reactor,
                                            "ssl:{}:privateKey={}:certKey={}".format(ssl_port, ssl_key, ssl_cert))
