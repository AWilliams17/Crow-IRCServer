...
class IRCProtocol(IRC):
        ...
        if not params:
            ...
            for user in self.users:
                if user == self.user_name:
                    self.sendToClient("* {} (** this is you), Room : {}".format(user,
                                                                                self.getRoom(user).name if self.getRoom(
                                                                                    user) else None))
                else:
                    self.sendToClient(
                        "* {}, Room : {}".format(user, self.getRoom(user).name if self.getRoom(user) else None))
            self.sendToClient("end of list.")
        else:
    ...
    def irc_MSG(self, prefix, params):
        ...
        if not current_room:
            self.sendToClient(
                "You can only send private message to a user in your room. Please join user's room to send them message.")
            return
        if not params:
